...
def main():
    parser = argparse.ArgumentParser(
        description="Make slices of XYZ image data for training. Assumes image is ZXYC unless specified otherwise using --channel_axis and --z_axis"
    )
    ...
    input_img_args.add_argument(
        "--all_channels", action="store_true", help="deprecated"
    )
    input_img_args.add_argument(
        "--anisotropy",
        required=False,
    ...
    if len(args.dir) > 0:
        image_names = io.get_image_files(
            args.dir, "_masks", imf=imf, look_one_level_down=args.look_one_level_down
        )
        dirname = args.dir
    else:
    ...
    for name in image_names:
        ...
        try:
            img0 = transforms.convert_image(
                img0, channel_axis=args.channel_axis, z_axis=args.z_axis, do_3D=True
            )
        except ValueError:
            print(
        ...
        for p in range(3):
            ...
            for k, img in enumerate(imgs):
                ...
                io.imsave(
                    os.path.join(dirname, f"train/{name0}_{npm[p]}_{k}.tif"),
