...
def _convert_image_3d(x, channel_axis=None, z_axis=None):
    ...
    if x.ndim < 3:
        raise ValueError(
            f"Input image must have at least 3 dimensions, input shape: {x.shape}, ndim={x.ndim}"
        )
    ...
    if channel_axis is None or channel_axis >= x.ndim:
        raise IndexError(
            f"channel_axis {channel_axis} is out of bounds for input array with {x.ndim} dimensions"
        )
...
def convert_image(x, channel_axis=None, z_axis=None, do_3D=False):
    ...
    if ndim == 2:
        ...
        x = x_out
        del x_out
        transforms_logger.debug(
            f"processing grayscale image with {x.shape[0], x.shape[1]} HW"
        )
    elif ndim == 3:
        ...
        del x_out
        transforms_logger.debug(
            f"processing image with {x.shape[0], x.shape[1]} HW, and {x.shape[2]} channels"
        )
    elif ndim == 4:
        ...
        del x_out
        transforms_logger.debug(
            f"processing image batch with {x.shape[0]} images, {x.shape[1], x.shape[2]} HW, and {x.shape[3]} channels"
        )
    else:
        # something is wrong: yell
        expected_shapes = "2D (H, W), 3D (H, W, C), or 4D (Z, H, W, C)"
        transforms_logger.critical(
            f"ERROR: Unexpected image shape: {str(x.shape)}. Expected shapes: {expected_shapes}"
        )
        raise ValueError(
            f"ERROR: Unexpected image shape: {str(x.shape)}. Expected shapes: {expected_shapes}"
        )
...
def random_rotate_and_resize(
    ...
    """
    scale_range = (
        max(0, min(2, float(scale_range))) if scale_range is not None else scale_range
    )
    nimg = len(X)
    if X[0].ndim > 2:
        nchan = X[0].shape[0]
