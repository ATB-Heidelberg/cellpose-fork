...
from cellpose.models import CellposeModel, assign_device, model_path, normalize_default
...
def add_noise(
    ...
    diams = diams if diams is not None else 30.0 * torch.ones(len(lbl), device=device)
    # ds0 = 1 if ds is None else ds.item()
    ds = (
        ds * torch.ones((len(lbl),), device=device, dtype=torch.long)
        if ds is not None
        else ds
    )
    ...
    if (ds is None and idownsample.sum() > 0.0) or not iso:
        ds = torch.ones(len(lbl), dtype=torch.long, device=device)
        ds[idownsample] = torch.randint(
            2, ds_max + 1, size=(idownsample.sum(),), device=device
        )
    ...
    if iblur.sum() > 0:
        if sigma0 is None:
            if uniform_blur and iso:
                ...
                sigma0 = diams[iblur] / 30.0 * gblur * xr[iblur]
                sigma1 = sigma0.clone() / 10.0
            else:
        ...
        if partial_blur:
            ...
            ym, xm = torch.meshgrid(
                torch.zeros(Ly, dtype=torch.float32),
            ...
            imgi[iblur, :, :, :Lxc] = (
                lbl_blur_crop * mask + (1 - mask) * imgi[iblur, :, :, :Lxc]
            )
        else:
            imgi[iblur] = lbl_blur
    ...
    for k in ii:
        i0 = (
            imgi[k : k + 1, :, :: ds[k], :: ds[k]]
            if iso
            else imgi[k : k + 1, :, :: ds[k]]
        )
    ...
    if ipoisson.sum() > 0:
        if pscale is None:
            ...
            pscale = torch.clamp(m.rsample(sample_shape=(ipoisson.sum(),)), 1.0)
...
def random_rotate_and_resize_noise(
    data,
    labels=None,
    diams=None,
    poisson=0.7,
...
def one_chan_cellpose(device, model_type="cyto2", pretrained_model=None):
    ...
    for name in net1.state_dict():
        if (
            "res_down_0.conv.conv_0" not in name
            and
            # "output" not in name and
            "res_down_0.proj" not in name
            and name != "diam_mean"
            and name != "diam_labels"
        ):
...
class CellposeDenoiseModel:
    ...
    def __init__(
        ...
        self.dn = DenoiseModel(
            gpu=gpu, model_type=restore_type, chan2=chan2_restore, device=device
        )
    ...
    def eval(
        ...
        masks, flows, styles = self.cp.eval(
            ...
            channel_axis=-1,
            z_axis=0
            if not isinstance(img_restore, list)
            and img_restore.ndim > 3
            and img_restore.shape[0] > 0
            else None,
            normalize=normalize_params,
            rescale=rescale,
            diameter=diameter,
...
class DenoiseModel:
    ...
    def __init__(
        ...
        if pretrained_model and (
            not isinstance(pretrained_model, str)
            and not isinstance(pretrained_model, Path)
        ):
        ...
        if model_type is not None or (
            pretrained_model and not os.path.exists(pretrained_model)
        ):
            pretrained_model_string = (
                model_type if model_type is not None else "denoise_cyto3"
            )
            ...
            self.diam_mean = 17.0 if "nuclei" in pretrained_model_string else 30.0
        else:
            if pretrained_model:
                builtin = False
    ...
    def eval(
        ...
        if isinstance(x, list) or x.squeeze().ndim == 5:
            ...
            nimg = len(x)
            iterator = (
                trange(nimg, file=tqdm_out, mininterval=30) if nimg > 1 else range(nimg)
            )
            imgs = []
            for i in iterator:
                imgi = self.eval(
            ...
            if np.ptp(x[..., -1]) < 1e-3 or (
                channels is not None and channels[-1] == 0
            ):
                x = x[..., :1]

            for c in range(x.shape[-1]):
                rescale0 = rescale * 30.0 / 17.0 if c == 1 else rescale
    ...
    def _eval(
        ...
        yf, style = run_net(self.net, img, bsize=bsize, tile_overlap=tile_overlap)
...
def train(
    ...
    optimizer = torch.optim.AdamW(
        net.parameters(), lr=learning_rate[0], weight_decay=weight_decay
    )
            ...
            if test_data is not None or test_files is not None:
                ...
                for ibatch in range(0, nimg_test_per_epoch, batch_size):
                    ...
                    if test_data is None:
                        imgs = [
                            np.maximum(0, io.imread(test_files[i])[:nchan])
                            for i in inds
                        ]
...
if __name__ == "__main__":
    ...
    model_args.add_argument(
        "--pretrained_model", default=[], type=str, help="pretrained denoising model"
    )
    ...
    training_args.add_argument(
        "--beta", default=0.7, type=float, help="scale of poisson noise"
    )
    training_args.add_argument(
        "--blur", default=0.0, type=float, help="fraction of images to blur"
    )
    training_args.add_argument(
        "--gblur", default=1.0, type=float, help="scale of gaussian blurring stddev"
    )
    training_args.add_argument(
        "--downsample", default=0.0, type=float, help="fraction of images to downsample"
    )
    training_args.add_argument(
        "--ds_max", default=7, type=int, help="max downsampling factor"
    )
    training_args.add_argument(
        "--lam_per", default=1.0, type=float, help="weighting of perceptual loss"
    )
    training_args.add_argument(
        "--lam_seg", default=1.5, type=float, help="weighting of segmentation loss"
    )
    training_args.add_argument(
        "--lam_rec", default=0.0, type=float, help="weighting of reconstruction loss"
    )
    training_args.add_argument(
        "--diam_mean",
        default=30.0,
    ...
    pretrained_model = (
        None if len(args.pretrained_model) == 0 else args.pretrained_model
    )
    model = DenoiseModel(
        gpu=True, nchan=1, diam_mean=args.diam_mean, pretrained_model=pretrained_model
    )
    ...
    nimg_test_per_epoch = (
        None if args.nimg_test_per_epoch == 0 else args.nimg_test_per_epoch
    )
    ...
    if isinstance(learning_rate, (list, np.ndarray)):
        ...
        if SGD:
            ...
            if model.n_epochs > 250:
                ...
                LR = np.append(
                    LR, model.learning_rate_const * np.ones(max(0, model.n_epochs - 10))
                )
        else:
    ...
    for iepoch in range(model.n_epochs):
        ...
        for ibatch in range(0, nimg_per_epoch, batch_size):
            ...
            imgi, lbl, scale = random_rotate_and_resize_noise(
                ...
                [train_labels[i][1:] for i in inds],
                poisson=poisson,
                blur=blur,
                downsample=downsample,
            ...
            if z_masking:
                nc = imgi.shape[1]
                nb = imgi.shape[0]
                ncmin = (np.random.rand(nb) > 0.25) * (
                    np.random.randint(nc // 2 - 1, size=nb)
                )
                ncmax = nc - (np.random.rand(nb) > 0.25) * (
                    np.random.randint(nc // 2 - 1, size=nb)
                )
                for b in range(nb):
                    imgi[b, : ncmin[b]] = 0
            ...
            if test_data is not None:
                ...
                for ibatch in range(0, len(test_data), batch_size):
                    ...
                    imgi, lbl, scale = random_rotate_and_resize_noise(
                        ...
                        [test_labels[i][1:] for i in inds],
                        poisson=poisson,
                        blur=blur,
                        downsample=downsample,
                        diams=diam_test[inds],
                        diam_mean=model.diam_mean,
                    )
                    imgi = imgi[:, :1]  # keep noisy only
