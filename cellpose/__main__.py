...
from cellpose import io, models, train, utils
from cellpose.cli import get_arg_parser

from .version import version_str

try:
    from cellpose.gui import gui, gui3d
...
def main():
    ...
    if args.train_size:
        logger.warning(
            "the '--train_size' flag is deprecated in v4.0.1+ and no longer used"
        )
    if args.chan or args.chan2:
        logger.warning(
            "--chan and --chan2 are deprecated, all channels are used by default"
        )
    if args.all_channels:
        logger.warning(
            "the '--all_channels' flag is deprecated in v4.0.1+ and no longer used"
        )
    if args.restore_type:
        logger.warning(
            "the '--restore_type' flag is deprecated in v4.0.1+ and no longer used"
        )
    if args.transformer:
        logger.warning(
            "the '--tranformer' flag is deprecated in v4.0.1+ and no longer used"
        )
    if args.invert:
        logger.warning(
            "the '--invert' flag is deprecated in v4.0.1+ and no longer used"
        )
    if args.chan2_restore:
        logger.warning(
            "the '--chan2_restore' flag is deprecated in v4.0.1+ and no longer used"
        )
    if args.diam_mean:
        logger.warning(
            "the '--diam_mean' flag is deprecated in v4.0.1+ and no longer used"
        )
    if args.train_size:
        logger.warning(
            "the '--train_size' flag is deprecated in v4.0.1+ and no longer used"
    ...
    if not args.train:
        _evaluate_cellposemodel_cli(
            args, logger, image_filter, device, pretrained_model, normalize
        )
...
def _train_cellposemodel_cli(
    ...
    if len(args.file_list) > 0:
        ...
        output = io.load_train_test_data(
            args.dir, test_dir, image_filter, args.mask_filter, args.look_one_level_down
        )
...
def _evaluate_cellposemodel_cli(args, logger, imf, device, pretrained_model, normalize):
    ...
    if not args.train:
        saving_something = (
            args.save_png or args.save_tif or args.save_flows or args.save_txt
        )
    ...
    for image_name in tqdm(image_names, file=tqdm_out):
        if args.do_3D or args.stitch_threshold > 0.0:
            ...
            if channel_axis or z_axis is None:
                ...
                image = io.imread(
                    image_name
                )  # Rely on transforms.convert_image() to move channels inside of .eval()
        ...
        if saving_something:
            ...
            if args.output_name is not None:
                ...
                elif args.savedir is not None and not os.path.samefile(
